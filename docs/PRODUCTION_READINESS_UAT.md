...
| F5 | Eval / regression suite of golden tasks | ⚠️ | Phase 27: static tripwires SHIPPED (`node scripts/agent-evals.mjs`, 30 assertions — every past incident pinned; run before push). Still missing: automated LIVE golden-task runs against the deployed app (G1–G5 documented in the script header, manual for now). |
