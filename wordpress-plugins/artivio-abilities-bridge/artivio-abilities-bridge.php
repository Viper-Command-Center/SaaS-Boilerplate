 ...
 *
 * v1.1.0 — every ability below now sets meta.mcp.public => true. Confirmed
 * directly against the MCP Adapter's own source (WP\MCP\Abilities\
 * McpAbilityExposure::is_meta_public()): an ability with no meta.mcp.public
 * AND no meta.public fails closed (treated as private) and is invisible to
 * discover-abilities, get-ability-info and execute-ability alike — being
 * registered in the Abilities API registry is not sufficient on its own.
 * This is why WordPress core's own abilities and WP Mail SMTP Pro's abilities
 * showed up in a live discover-abilities test on build-6 while these five did
 * not: those plugins set the flag, v1.0.0 of this plugin never did. No
