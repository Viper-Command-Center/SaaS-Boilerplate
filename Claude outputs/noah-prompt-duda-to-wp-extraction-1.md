...
You are extracting everything needed to rebuild `{{SITE_NAME}}` on WordPress (Gutenberg + Kadence Blocks), as faithfully as possible to how it currently looks and reads. This is an **extraction task only** — you are not building anything in WordPress in this task, and you must not modify anything in DUDA. Output is one structured content package that a separate build task will consume.
...
3. `get_site_theme` — the full color palette (note the semantic `label` on each color, e.g. "Accent Gold" — carry these labels forward, they map onto Kadence's Global Palette entries) and typography per heading level, button styles, and section spacing. Record this verbatim; it's the single most useful thing the API gives you and maps almost directly onto Kadence's Global Palette / Global Typography settings (set once site-wide, referenced by every block — not per-block values).
...
- **This feeds directly into the Gutenberg/Kadence playbook** — `design_tokens` maps onto Kadence's Global Palette / Global Typography (set once in Appearance → Customize, referenced by every block via `var(--global-paletteN)`, not hardcoded per block). Each `pages[].sections[]` entry maps onto one Kadence `rowlayout`/`column` block pair: `heading` → an `advancedheading` block, `body_text` → a `paragraph` block, `images` → a Kadence image/gallery block — each new block needs its own freshly generated `uniqueID`, and a cache/recompile trigger after the write (see the plugin playbook's builder section). Worth writing the "rebuild" prompt as a second task that consumes this JSON directly, rather than describing it from scratch each time.
